...
for i in range(len(archi_features_monumenai)):
    ...
    for k in range(len(list(MonMonumenAI.ELEMENT_DIC.keys()))):
        style = list(MonMonumenAI.ELEMENT_DIC.keys())[k]
        if local_el in MonMonumenAI.ELEMENT_DIC[style]:
...
element_dic_monumenai = MonMonumenAI.ELEMENT_DIC
